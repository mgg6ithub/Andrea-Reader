    ...
    var body: some View {
        ...
        let data: [ReadingProgress] = {
            ...
            if estadisticas.sesionesLectura.count == 1,
               ...
               let fin = sesion.fin {
                ...
                result.append(ReadingProgress(date: fin, actual: actualFin, ideal: idealFin))
            }
            return result
        }()
        
        // Fechas para ticks cada hora en modo detallado
        ...
        Chart {
            ...
            ForEach(data) { item in
                LineMark(
                    ...
                    y: .value("Progreso", min(max(item.actual, 0), 100)),
            ...
            ForEach(data) { item in
                LineMark(
                    x: .value("Fecha", item.date),
                    y: .value("Progreso", min(max(item.ideal, 0), 100)),
        ...
        .chartXAxis {
            if verTodo {
                ...
                // Modo resumido: ticks automáticos
